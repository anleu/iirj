...
public class IIRJScipyCompare
{
	...
	private void compareSignals(DoubleSignal scipySignal, DoubleSignal iirjSignal) {
		...
		if (signal2Size != signal1Size) {
			Assert.fail("Different signal1Size of signals: " + signal1Size + " vs. " + signal2Size);
		}
		for (int index = 0; index < signal1Size; index++) {
			...
			if (compareDoublesWithPrecision(scipySignalValue, iirjSignalValue, 5) != 0) {
				Assert.fail("Difference values at index: " + index + " | Scipy: " + scipySignalValue + " | IIRJ: " + iirjSignalValue);
			}
		}
	}

	public int compareDoublesWithPrecision(double d1, double d2, int decimalPlaces) {
		if (decimalPlaces < 0) {
			throw new IllegalArgumentException("Number of decimal digits has to be positive.");
		}

		long factor = (long) Math.pow(10, decimalPlaces);
		long aAdjusted = Math.round(d1 * factor);
		long bAdjusted = Math.round(d2 * factor);

		return Long.compare(aAdjusted, bAdjusted);
	}
